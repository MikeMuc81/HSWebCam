...
// icon-color: blue; icon-glyph: video;
...
// ⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺
// With widget parameter you can select one of the multiple weCams provided by Hessenschau
// parameter has to be equal to the id given in the array below (e.g. "Herkules" or "bsa" - without quotes)
// 
// ToDo / Ideas
// ⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺
// 
...
let webCams = [
   ...
   {id:"dillenburg", label: "Wilhelmsturm",  url:"https://www.hessenschau.de/wetter/wetterkameras/wetterkamera-dillenburg-wilhelmsturm-100.html", lat: 50.7404535, lon: 8.2874957, adjust: "bottom", showNightPics: false},
   {id:"frankfurt", label: "Skyline",  url:"https://www.hessenschau.de/wetter/wetterkameras/index.html", lat: 50.1106444, lon: 8.6820917, adjust: "no", showNightPics: true},
   {id:"fulda", label: "St. Blasius",  url:"https://www.hessenschau.de/wetter/wetterkameras/wetterkamera-fulda-100.html", lat: 50.5521486, lon: 9.676511, adjust: "no", showNightPics: false},
   {id:"marburg", label: "Schlossberg",  url:"https://www.hessenschau.de/wetter/wetterkameras/wetterkamera-marburg-schloss-panorama-100.html", lat: 50.8117327, lon: 8.7757742, adjust: "no", showNightPics: false},
   {id:"ruedesheim", label: "Rüdesheim",  url:"https://www.hessenschau.de/wetter/wetterkameras/wetterkamera-ruedesheim-am-rhein-100.html", lat: 49.84470965, lon: 7.80447218250423, adjust: "bottom", showNightPics: false},
   {id:"edersee", label: "Edersee",  url:"https://www.hessenschau.de/wetter/wetterkameras/wetterkamera-schloss-waldeck-edersee-100.html", lat: 51.20381495, lon: 9.0553826791216, adjust: "bottom", showNightPics: false},
   {id:"wasserkuppe", label: "Wasserkuppe",  url:"https://www.hessenschau.de/wetter/wetterkameras/wetterkamera-wasserkuppe-100.html", lat: 50.497647, lon: 9.9362893, adjust: "bottom", showNightPics: false},
   {id:"wiesbaden", label: "Bowling Green",  url:"https://www.hessenschau.de/wetter/wetterkameras/wetterkamera-wiesbaden-kurhaus-100.html", lat: 50.0820384, lon: 8.2416556, adjust: "bottom", showNightPics: false},
   {id:"willingen", label: "Lohkopf",  url:"https://www.hessenschau.de/wetter/wetterkameras/wetterkamera-willingen-100.html", lat: 50.6671285, lon: 8.0869407, adjust: "bottom"}
];
...
async function createWidget(items) {
  ...
  parseInput(args.widgetParameter)
  
  var webCam = webCams.filter(webCamObj => webCamObj.id.indexOf(locationID) > -1)
  if (webCam.length == 0) { 
    list.addText("⚠︎ Parameter")
    list.addText("     nicht")
    list.addText("     korrekt!")
    return list
  } 

  let lastImgFile    = webCam[0].id + "WebCam.jpg"
  let lastTimeFile   = webCam[0].id + "WebCam.time"
  let path = fm.joinPath(dir, lastTimeFile)

  list.setPadding(0,14,2,14)
  list.url = webCam[0].url
  
  // fetch website
  webPage = await loadWebPage(webCam[0].url);
  ...
  // ... and yesterday
  civil_twilight(yesterday.getFullYear(), yesterday.getMonth()+1, yesterday.getDate(), webCam[0].lon, webCam[0].lat, startCivilTwilight, endCivilTwilight)
  ...
  if (webCam[0].showNightPics == false) {
      ...
      if ( (picDate > riseDate          && picDate < sunsetDate         ) ||
        ...
        if (lastImg != null) {
          ...
          if (imgList[i][1] == lastImg.url) {
            img = await loadWebImage("", lastImgFile)  // image from cache
          }
          // image from web as cached image is outdated
          else {
            img = await loadWebImage(imgList[i][1], lastImgFile)
    ...
    if (i == imgList.length) {
      img = await loadWebImage("", lastImgFile)
      timeStr = dfTime.string(new Date(lastImg.timeStr))    
    }
  } 
  // get the latest img
  else {
    img = await loadWebImage(imgList[0][1], lastImgFile)
    timeStr = dfTime.string(new Date(imgList[0][0]))
  }

  if ( /*config.widgetFamily == 'medium' &&*/ webCam[0].adjust.indexOf("no") == -1 ) {
    img = adjustImage(img, webCam[0].adjust)
  }
...
// crop image
function adjustImage(img, adjust) {
  ...
  if (adjust.indexOf("top") > -1)
  { 
    draw.drawImageAtPoint(img, new Point(0, 0))    
  } else if (adjust.indexOf("bottom") > -1) {
    draw.drawImageAtPoint(img, new Point(0, rect.height - img.size.height))  
  }
...
async function loadWebImage(imgUrl, imgFileName) {
...
function parseInput(input) {
  ...
  if (input != null && input.length > 0) {
    ...
    if (parCount > 0) { locationID = wParameter[0].trim().toLowerCase() }
  }
